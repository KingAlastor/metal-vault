  ...
  instance.interceptors.response.use(
    ...
    async (error) => {
      if (error.response?.status === 403 || error.response?.status === 429) {
        console.log(`Rate limited (${error.response.status}) - waiting before retry...`);
    ...
    for (const { id, name, archivesLink } of archivesLinks) {
      ...
      while (retryCount < maxRetries && !success) {
        try {
          ...
          for (const elem of rows) {
            ...
            if (href) {
              ...
              if (name && albumName && albumLink > 0) {
                ...
                const releaseData = await getReleaseData(albumUrl, axiosInstance);
                if (releaseData) {
                  const albumData = {
                    band_id: id,
                    name: albumName,
          ...
          retryCount++;
          console.error(`Error processing band ${name} (${id}) - Attempt ${retryCount}:`, error);
          ...
          if (axios.isAxiosError(error)) {
            ...
            if (status === 403 || status === 429) {
              ...
              const waitTime = Math.min(30000 * Math.pow(2, retryCount), 300000); // Max 5 minutes
              await new Promise((resolve) => setTimeout(resolve, waitTime));
            } else if (status === 404) {
